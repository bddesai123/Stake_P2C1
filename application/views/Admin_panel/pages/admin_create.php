 ...
 <?php 
    ...
    <div class="col-md-4 card bg-light">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
             <button type="submit" class="btn btn-dark shadow-dark px-5 btn-remove" name="admin_btn"><i class="icon-lock"></i> Add Subadmin</button>
           </div>
           </form>
        </div>
    </div>
    ...
    <table class="table">
        ...
        if($all_subadmin){
            ...
            foreach($all_subadmin as $subadmin){
               ...
               $datas=json_decode($subadmin->rights);
               
                ?>
                <tr>
                    <td><?= $sno;?>  </td>
                    <td><?= $subadmin->user;?>   </td>
                    <td><a onclick="return confirm('Are you sure you want to delete this item')" href="<?= $admin_path.'admin/delete_admin?id='.$subadmin->id;?>" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i>Delete</a>
                    <a href="<?= $admin_path.'admin/edit_admin?id='.$subadmin->id;?>" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>Edit</a>
                    <!--<a class="btn btn-info btn-sm" href="<?= $admin_path.'admin/login?user='.$subadmin->id;?>" target="_blank">Login </a>-->
                    </td>
                   <td><?php
                   foreach($datas as $datasd){
                       ?>
                    <p><?= $datasd;?></p>
                    <?php
                   }
