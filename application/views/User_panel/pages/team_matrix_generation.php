           ...
           <center>
            ...
            <div class="form-inline1 col-xl-7">  
               ...
               <?php 
                ...
                $total_all_users=$this->conn->runQuery('id','pool',"u_id='$user_id'");
                
               ?>
                <form action="<?= $panel_path.'team/team-matrix-generation';?>" class="form-inline my-2 my-lg-0" method="post">
    
     <label style="color:black;">Total Entry</label>
			...
			<select name="select_pool" class="form-control" id="select_pool" >
			   ...
			   if($total_all_users){
			       $sr=1;
			     foreach($total_all_users as $total_all_users1){
			   ?> 
			 <option value="<?= $total_all_users1->id;?>" <?php echo ($this->session->has_userdata('user_pool_id') && $this->session->userdata('user_pool_id')==$total_all_users1->id ? "selected":0);?>><?= $sr;?></option>	
				<?php 
				$sr++;
				}} ?>
			</select>
             ...
             <?php
            	 if($this->session->has_userdata('user_pool_id')){
            					$user_pool_id=$this->session->userdata('user_pool_id');
