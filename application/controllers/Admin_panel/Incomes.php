...
class Incomes extends CI_Controller{
    ...
    public function index(){
        ...
        if(isset($_REQUEST['export_to_excel'])){
            ...
            $sourceexport=$_POST['export_income_source'];
          
           $whr="tx_type='income' and source='$sourceexport'";
           ...
           if($get_data){
               ...
               for($f=0;$f<count($get_data);$f++){
                ...
                $from_tx_profile=$this->profile->profile_info($get_data[$f]->tx_u_code);
                $bank_details=json_decode($get_data[$f]->bank_details);
               
                $dta['Tx User']=$tx_profile->username.'( '.$tx_profile->name.')';
                $dta['From User']=$from_tx_profile->username.'( '.$from_tx_profile->name.')';
                $dta['Credit/Debit']=$get_data[$f]->debit_credit;
