...
class Register extends CI_Controller{
        ...
        if(isset($_POST['register'])){
            ...
            $sponsor_id=$register['u_sponsor'];
             ...
             if(array_key_exists('user_gen_method', $value_by_lebel) && $value_by_lebel['user_gen_method']=='manual'){
                 $this->form_validation->set_rules('usename', 'Usename', 'required|min_length[6]|alpha_numeric|trim|callback_is_username_valid');
	...
	public  function register_new(){
        if(isset($_POST['address']) && $_POST['address']!='' ){
            ...
            $details  =  $this->conn->runQuery('id','users',"user_address='$address'");  
            $register['user_address']=$address;			
            if($details){ 			
                $res['error']=true; 
				...
				if(isset($_POST['sponsor']) && $_POST['sponsor']!='' ){
					...
					$sp_details_id  =  $this->conn->runQuery('id','users',"username='$sponsor'");
					if($sp_details_id ){
						
						$sp_id=$sponsor_id=$sp_details_id[0]->id;
						$register['u_sponsor']=$sp_id;		
						////////////////////////////////////////////////////////////////
						$register['username']=$user_code=$this->get_username();
				...
				// 		 $register['position'] = $_POST['position'];
                         $register['Parentid'] = $this->binary->new_parent($sp_id,$_POST['position']);
            ...
            $res['error']=true;            
            $res['msg']="Please connect to TokenPocket and switch to polygon Network";
        } 
	...
	public  function username_available(){
		...
		if(isset($_POST['username']) && $_POST['username']!='' ) {		
            $user=$_POST['username']; 		
			$details=$this->conn->runQuery('id,active_status,username','users',"user_address='$user'");
			if($details){
				
			    $res['error']=false;
				$res['msg']="Success";
				$res['code']=$details[0]->username;
