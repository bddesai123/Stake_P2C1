...
function direct_roi_destribute($u_code,$amount,$no_of_levels=10){
     $code=$u_code;
        $ben_from=$u_code;
        $incomes=array();
    ...
    $username=$profile_info->username;
        ...
        $plan=$this->conn->runQuery('referral_income,id','plan','1=1');
        if($plan){
            $referral_income=array_column($plan,'referral_income','id');
            while($code!=''){
                $source =($l==1 ? 'referral_income' : "referral_income");
               $currenct_payout_id=$this->wallet->currenct_payout_id();
               
                    $level_per=$referral_income[$l];
                ...
                if($payable>0){     
				    $order_amount=$this->business->package($code);
				    $cappings=$order_amount*3;
				    $total_amountss=$this->conn->runQuery('SUM(amount) as amts','transaction',"u_code='$code' and tx_type='income' and (source='$source' ) ")[0]->amts;
				    
				    if ($query->num_rows() > 0) {
                    $row = $query->row();
                    $tx = $row->id;
                    $payable21=$payable+$total_amountss;
                    if($cappings>$payable21){
                        $payables=$payable;
                        $flash=0;
                    }else{
                        $payable=$cappings-$total_amountss;
                ...
                if($payable>0 && $code!='' ){
                    ...
                    $income['tx_record']=$l;
                    $income['remark']="Recieve Nested ROI income of amount $payable from $name ($username) from level $l ";
                    if($this->db->insert('transaction',$income)){
                        echo "<br><br>code=".$code;
                        echo "<br>tx=".$tx;
                        echo "<br>pay".$l."=".$payable;
                        $income_lvl=$income['amount'];//-$income['tx_charge'];
                        $this->update_ob->add_amnt($code,'referral_income',$income_lvl);
...
function level_destribute($u_code,$amount,$no_of_levels=1){
        $code=$u_code;
        $ben_from=$u_code;
        $incomes=array();
        $l=1;
            ...
            while($code!=''){
               ...
               $currenct_payout_id=$this->wallet->currenct_payout_id();
                ...
                if($payable>0){     
				    ...
				    $cappings=$order_amount*3;
				    $total_amountss=$this->conn->runQuery('SUM(amount) as amts','transaction',"u_code='$code' and tx_type='income' and (source='$source' ) ")[0]->amts;
				    $tx=$this->conn->runQuery('id',orders,"u_code='$code'")[0]->id;
                    $payable21=$payable+$total_amountss;
                    if($cappings>$payable21){
                        $payables=$payable;
                        $flash=0;
                ...
                if($payable>0 && $code!='' ){
                    ...
                    $income['status']=1;
                    $income['payout_id']=$currenct_payout_id;
                    $income['wallet_type']='refferal_wallet';
                    $income['tx_record']=$l;
    ...
    public function distribute_pool_level_rev($p_id,$amount,$no_of_levels=16){
        $pol_id='1';
        ...
        if($details){
            ...
            $team_requried=array_column($plan,'matrix_team_required','id');
            $rank=array_column($plan,'package_name','id');
            $rank_royalty=array_column($plan,'package_name','id');
            ...
            for($l=1;$l<=$no_of_levels;$l++){
                ...
                if($pdetails){
                    ...
                    $payble=1;
                    $req_team=$team_requried[$l];
                  
                    $my_rank=$rank[$l];
                    $rank_royalty_type=$rank_royalty[$l];
                    $all_team=$this->team->my_pool_team($ids,'1',20);
                    $bronze_lvl1=(!empty($all_team) ? count($all_team[$l]):0);
                    if($bronze_lvl1==$req_team){
                        $check_user_detail=$this->conn->runQuery('*','rank',"u_code='$code' and is_complete='1' and rank='$my_rank'");
                       ...
                       if($payble>0 && $payble!=''){
                                ...
                                $this->update_ob->add_amnt($code,$source,$income_lvl);
                                $this->update_ob->add_amnt($code,'autopool_wallet',$income_lvl);
     ...
     public function gen_pool($u_code,$top_amt=0){
        $pool_typ='pool1';
        $get_matrix_parent=$this->binary->get_pool_parent(1,$pool_typ);
        ...
        $update_position['pool_id']=1;
        $pidd=$this->conn->get_insert_id('pool',$update_position);
        $this->distribute->distribute_pool_level_rev($pidd,$top_amt,20);
